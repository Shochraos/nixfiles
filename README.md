...
`nix flake check` runs seven checks: `treefmt`, a unit suite, one behaviour check per shipped script, and a `pre-commit` check that runs the same two hooks a commit does.
